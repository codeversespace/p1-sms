...
MSG_FORMAT = '[%(asctime)s.%(msecs)03dZ] [%(levelname)s] [%(app_name)s] [%(app_code)s] [SMS] [%(module)s:%(lineno)d] %(message)s'
...
class SmsLogger:
    ...
    def __getFileNameWithPath(self, filename: str = None, location: str = None) -> str:
        ...
        filename =f'log_{datetime.datetime.now()}'
...
logger = SmsLogger().name('nest-test-engine').fileName('nest-work-engine').level(logging.DEBUG).getRootLogger()
